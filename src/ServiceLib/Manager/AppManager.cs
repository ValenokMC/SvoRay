...
public sealed class AppManager
{
    ...
    public bool InitApp()
    {
        if (Utils.IsWindows())
        {
            // SvoRay always keeps user data in %LOCALAPPDATA%\SvoRay.
            // Checking write permission is not enough here: the client runs elevated for
            // TUN, so it could write into Program Files even though it must not.
            Environment.SetEnvironmentVariable(Global.LocalAppData, "1", EnvironmentVariableTarget.Process);
        }
        else if (Utils.HasWritePermission() == false)
        {
            Environment.SetEnvironmentVariable(Global.LocalAppData, "1", EnvironmentVariableTarget.Process);
        }
...
#pragma warning restore CS0618
    ...
    /// <summary>
    /// Earlier builds shipped under a different product name and, before 0.2.0, kept their
    /// settings next to the EXE. This is the only place that still needs the former name:
    /// without it, existing users would silently lose their profiles after the rename.
    /// </summary>
    private const string FormerAppFolderName = "SvobodaStreet";

    /// <summary>
    /// Brings settings written by an earlier build into %LOCALAPPDATA%\SvoRay, from the
    /// portable folder next to the EXE and from the previous product's data folder.
    /// Runs once: each folder is skipped as soon as the target already holds it. Logs and
    /// generated core configs are not migrated - they are regenerated anyway and would
    /// only carry stale state across.
    /// </summary>
    private static void MigrateLegacyPortableData()
    {
        var target = Utils.StartupPath();

        MigrateDataFolders(Utils.GetBaseDirectory(), target, "portable");

        var formerLocalAppData = Path.Combine(
            Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),
            FormerAppFolderName);
        MigrateDataFolders(formerLocalAppData, target, "previous version");
    }

    private static void MigrateDataFolders(string source, string target, string description)
    {
        try
        {
            if (!Directory.Exists(source) || string.Equals(
            ...
            foreach (var folder in new[] { "guiConfigs", "guiBackups" })
            {
                var from = Path.Combine(source, folder);
                var to = Path.Combine(target, folder);
                if (!Directory.Exists(from) || Directory.Exists(to))
                {
                    continue;
                }

                FileUtils.CopyDirectory(from, to, true, false);
                Logging.SaveLog($"Migrated '{folder}' from the {description} data folder.");
            }
        }
        catch (Exception ex)
        {
            // A failed migration must never block start-up: the app then simply starts fresh.
            Logging.SaveLog("MigrateLegacyData", ex);
        }
    }
